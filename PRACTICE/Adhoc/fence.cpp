...
int main(){
    ll N,q; cin >> N >> q;
    vector<pair<Point,ll> > point(N);
    for(ll i=0;i<N;i++){
        cin >> point[i].first.x >> point[i].first.y;
        point[i].second=i;
    }
    vector<pair<Point,ll> > cp(point);
    sort(point.begin(),point.end(),__cmp);
    vector<ll> fenceId(N,0);
    ...
    for(ll Q=1;Q<=q;Q++){
        ...
        if(str=="query"){
            ...
            i=*binary_search(point.begin(),point.end(),cp[i]);
            j=*binary_search(point.begin(),point.end(),cp[j]);
         (fenceId[i]==fenceId[j])?cout<<"YES"<<endl:cout<<"NO"<<endl;
            ...
            ll LId = 0,HId=N-1;
            bool check=false;
            lowerBoundX(point,0,N-1,x1,x2,LId,check);
            higherBoundX(point,0,N-1,x1,x2,HId,check);
